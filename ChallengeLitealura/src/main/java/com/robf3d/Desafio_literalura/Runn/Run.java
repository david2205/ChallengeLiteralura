package com.robf3d.Desafio_literalura.Runn;
import com.robf3d.Desafio_literalura.models.*;
...
import com.robf3d.Desafio_literalura.service.APIConsume;
import com.robf3d.Desafio_literalura.service.DataTransform;
...
public class Run {
    ...
    APIConsume consumoAPI = new APIConsume();
    DataTransform conversor = new DataTransform();

    private LibroRepository lRepository;
    private AutorRepository ARepository;
    public Run(LibroRepository libroRepository , AutorRepository autorRepository) {
    ...
    private void listarLibrosPorIdioma() {
        ...
        switch (idiomaSeleccionado){
            case 1:
                List<Book> libroBuscadoEs=lRepository.obtenerLibroPorIdiomaEs();
                libroBuscadoEs.forEach(l -> System.out.println(l));
                break;
            case 2:
                List<Book> libroBuscadoEn=lRepository.obtenerLibroPorIdiomaEn();
                libroBuscadoEn.forEach(l -> System.out.println(l));
                break;
            case 3:
                List<Book> libroBuscadoFr=lRepository.obtenerLibroPorIdiomaFr();
                libroBuscadoFr.forEach(l -> System.out.println(l));
                break;
            case 4:
                List<Book> libroBuscadoPr=lRepository.obtenerLibroPorIdiomaPr();
    ...
    private void ListarAutoresVivos() {
        ...
        List<Writer> autoresBuscados = ARepository.obtenerAutoresVivos(autorVivo);
    ...
    private void ListarAutoresRegistrados() {
        List<Book> libroBuscado=lRepository.obtenerTodosLosLibros();
        List<Writer> autoresBuscados = ARepository.obtenerTodosLosAutores();
        if(autoresBuscados.size()>0){
            System.out.println("\n" + "-----Autores registrados-----" + "\n");
    ...
    private void buscarlibro() {
        ...
        if(busqueda != null) {
            ...
            var autor = new Writer(primerAutor.nombre(),
                    primerAutor.fechaDeNacimiento(),
                    primerAutor.fechaDeMuerte()
            );

            var libro = new Book(busqueda.titulo(),
                    busqueda.idiomas(),
    ...
    public BookDates BuscarlibroJson(String busqueda){
        var json = consumoAPI.obtenerDatos(URL_INICIO + "?search=" + busqueda);
        ResultDates datos = conversor.obtenerDatos(json,ResultDates.class);
